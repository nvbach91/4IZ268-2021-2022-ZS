...
const API_KEY = '?api_key=24d519ba38eaacef95f5c46bc71f2996';

const SEARCH_INPUT_BOX = document.getElementById('search-input-box');
const CLEAR_LOCAL_STORAGE_BUTTON = document.getElementById('clear-local-storage');
const SEARCH_RESULTS_CONTAINER = document.getElementById('search-results-container');
const SAVED_SHOWS_CONTAINER = document.getElementById('saved-shows-container');
const LOADING = document.getElementById('loading');
const LOGO = document.getElementById('logo');
const DUMMY_DATA_BUTTON = document.getElementById('insert-dummy-data');

$(function () {
    SEARCH_RESULTS_CONTAINER.style.display = 'none';
    SEARCH_INPUT_BOX.addEventListener("input", (event) => {
    ...
    CLEAR_LOCAL_STORAGE_BUTTON.addEventListener('click', () => {
        localStorage.clear();
        document.title = 'Series Tracker';
    ...
    DUMMY_DATA_BUTTON.addEventListener('click', () => {
        setLoading(true);
        localStorage.clear();
        document.title = 'Series Tracker';
        $.ajax({
...
function clearSearchResultsContainer() {
    while (SEARCH_RESULTS_CONTAINER.firstChild) {
        SEARCH_RESULTS_CONTAINER.removeChild(SEARCH_RESULTS_CONTAINER.firstChild);
    }
}

function clearSavedShowsContainer() {
    while (SAVED_SHOWS_CONTAINER.firstChild) {
        SAVED_SHOWS_CONTAINER.removeChild(SAVED_SHOWS_CONTAINER.firstChild);
    }
}
...
function runSearch(keyword) {
    $.ajax({
        type: 'GET',
        url: BASE_SEARCH_URL + API_KEY + '&query=' + keyword,
        success: function (data) {
            let tvShowsArray = [];
...
function renderSearchResults(tvShowsArray) {
    ...
    SEARCH_RESULTS_CONTAINER.appendChild(showTilesWrapper);
}
...
function createSearchResultTile(tvShow) {
    ...
    let searchResultPoster = document.createElement('img');
    if (tvShow.poster_path === null) {
        searchResultPoster.setAttribute('src', 'assets/no_poster.png');
        searchResultPoster.setAttribute('alt', 'Blank poster');
    } else {
        searchResultPoster.setAttribute('src', BASE_IMAGE_URL + tvShow.poster_path);
        searchResultPoster.setAttribute('alt', 'Poster of ' + tvShow.name);
    }
    ...
    searchResultWrapper.appendChild(searchResultPoster);
...
function getEpisodes(tvShowData, numberOfSeasons, showName) {
    ...
    function getEpisodesOfSeason(seasonIndex, seasonNumber, numberOfSeasons) {
        $.ajax({
            type: 'GET',
            url: BASE_URL + tvShowData.id + '/season/' + seasonNumber + API_KEY,
            success: function (data) {
...
function renderSavedShowTiles() {
    ...
    Object.keys(localStorage).forEach(function (tvShowId) {
        ...
        SAVED_SHOWS_CONTAINER.appendChild(createTvShowTile(tvShowData));
    });
}
...
function createTvShowTile(tvShowData) {
    ...
    if (tvShowData.poster_path === null) {
        ...
        savedShowPoster.setAttribute('src', BASE_IMAGE_URL + tvShowData.poster_path);
...
function renderTvShowDetails(tvShowData) {
    ...
    SAVED_SHOWS_CONTAINER.appendChild(savedTvShowWrapper);
    setLoading(false);
    document.title = tvShowData.name;
}
...
function renderError() {
    ...
    SAVED_SHOWS_CONTAINER.appendChild(div);
}
...
function setIntroductoryNotes() {
    ...
    SAVED_SHOWS_CONTAINER.appendChild(notesWrapper);
}