...
var countryArr = []; // array for the contry options
const keys = Object.keys(obj); // object keys
var prop = keys[Math.floor(Math.random() * keys.length)]; // random object property
var isCorrect; // variable that determines if the country was guessed correctly
let loadImg;
...
// load the images on options once the apllication is ready
$(document).ready(function () {
    let startWindow = $('#startWindow');
    startWindow.css('display', 'block');
    let layout = $('#layout');
    layout.css('display', 'none');
    let gameOverWindow = $('#gameOverWindow');
...
function loadImage() {
    loadImg = $('#flag-image').attr('src', `https://flagcdn.com/${prop}.svg`);
};
...
function loadAnswers() {
    ...
    for (let i = 0; i < 5; i++) {
        var rndm = obj[keys[Math.floor(Math.random() * keys.length)]];
