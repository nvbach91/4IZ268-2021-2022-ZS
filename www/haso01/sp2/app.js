$(document).ready(() => {
	...
	/*fetch('https://geolocation-db.com/json/').then((resp) => {
		return resp.json();	
	}).then((resp) => {
		const cityValue = {
			city: resp.city
		...
		cityNameInput.val(cityValue.city);
	})*/
	...
	function getPosition(position) {
		...
		fetch('http://api.openweathermap.org/geo/1.0/reverse?lat=' + lat + '&lon=' + long + '&limit=5&appid=49e168c62ed9de1a220b5ebdbeca7b56').then((resp) => {
			return resp.json();
		}).then((resp) => {
			const cityValue = {
				name: resp[0].name
			...
			cityNameInput.val(cityValue.name);
			weatherForm.submit();
		})
	}
		...
		fetch('https://api.openweathermap.org/geo/1.0/direct?q=' + inputValue + apiKey).then((resp) => {
			...
			fetch('https://api.openweathermap.org/data/2.5/onecall?lat=' + city.lat + '&lon=' + city.lon + '&exclude=minutely,hourly,alerts&units=metric' + apiKey).then((resp) => {
				...
				$.each(daily, function (i, jsonObject) {
					...
					dailyStats.push(dailyWeatherContainer);
					//dailyWeatherStats.append(dailyWeatherContainer);
				})
				dailyWeatherStats.append(dailyStats)
				daily = [];
