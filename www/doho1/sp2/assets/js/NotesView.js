...
export default class NotesUI {
    ...
    _createListItemHTML(id, title, body, time, temp) {
        ...
        return `
            <div class="notes-list-item" data-note-id="${id}">
                ...
                <div class="notes-item-time" data-note-id="${id}">
    ...
    updateNoteList(notes) {
        ...
        for (const note of notes) {
            const html = this._createListItemHTML(note.id, note.title, note.body, new Date(note.time), note.temp);
            tempHtml.push(html);
        }
        notesListContainer.insertAdjacentHTML("beforeend", tempHtml.join(''));
    ...
    updateNotePreviewVisibility(visible) {
        this.root.querySelector(".notes-wrapper").style.visibility = visible ? "inherit" : "hidden";
    }
}