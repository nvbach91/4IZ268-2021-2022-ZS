...
function shiftString(str, shift) {
    ...
    for (let i = 0; i < str.length; i++) {
        decodedString += shiftChar(str.charAt(i), shift);
    }
...
function caesarDecipher(cipherText, usedKey) {
    ...
    for (let i = 0; i < cipherText.length; i++) {
        if (alphabet.indexOf(cipherText.charAt(i)) != -1) {
            remainingChar += cipherText.charAt(i);
        } else {
            resultMessage += shiftString(remainingChar, usedKey) + cipherText.charAt(i);
            remainingChar = '';
        }
    }
    if (remainingChar.localeCompare('') != 0) {
        resultMessage += shiftString(remainingChar, usedKey);
    }
...
const textArea = document.getElementById('cipher');
...
but.addEventListener('click', () => {
    ...
    const resultMessage = caesarDecipher(textArea.value, inpKey.value);
