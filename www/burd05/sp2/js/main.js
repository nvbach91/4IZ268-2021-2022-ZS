...
const apiKey = 'ca041f3f70c48f587f62d88206b6471b';
...
function showTasks(sortByNames = false, sortByPriority = false, sortByDate = false) {
  ...
  if (localStorage.length > 0) {
    ...
    myTasks.forEach((element, index) => {
      ...
      if (element.done) newLiTag += '<li class="disableLi" onclick="viewTask(' + index + ')">' + element.name + '<span class="icon" onclick="deleteTask(' + index + ')"><img src="img/trash.png"></span></li>';
      ...
      myTasks.forEach((element) => {
        ...
        fetch(
          ...
          if (element.done) {
            ...
            forecast.textContent = data.current.weather[0].description;

          } else if (element.datetime > todayPlus7.toISOString().slice(0, 16)) {

            temperature.textContent = 'není předpověd';
            forecast.textContent = 'není předpověd';

          } else {

            const diffTime = Math.abs(todayPlus7 - new Date(element.datetime));
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
            let index;
            if (diffDays == 7) index = 1;
            if (diffDays == 6) index = 2;
            if (diffDays == 5) index = 3;
            if (diffDays == 4) index = 4;
            if (diffDays == 3) index = 5;
            if (diffDays == 2) index = 6;
            if (diffDays == 1) index = 7;

            let { day } = data.daily[index].temp;
            temperature.textContent = day + ' °';
            forecast.textContent = data.daily[index].weather[0].description;
            console.log(data);
          }
        })
