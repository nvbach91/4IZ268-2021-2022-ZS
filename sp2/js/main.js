...
function getPlayerData(accountID) {
  fetch(`https://api.opendota.com/api/players/${accountID}`)
    .then((response) => response.json())
    .then((responseJSON) => showPlayerData(responseJSON, accountID));
};
...
function displaySearchResults(data) {
  ...
  data.sort((a, b) => {
    return a.last_match_time < b.last_match_time
      ? 1
      : b.last_match_time < a.last_match_time
      ? -1
      : 0;
  });

  let resultsAmount = 1;
  if (data.length < resultsAmount) {
  ...
  for (let i = 0; i < resultsAmount; i++) {
    ...
    getRecentMatches(data[i].account_id);
  };

  for (let i = 0; i < resultsAmount; i++) {
    searchResults += `<div class="left">
    <div class="player-name">${data[i].personaname}</div>
    <img class = "avatar" src="${data[i].avatarfull}" alt="avatar">
  </div>`;
  };
  searchResults += `</ul>`;
...
function showPlayerData(data) {
  $("main").append(`<div class="right">
                      <ul>
                        ...
                        <li>Nationality: ${
                          data.profile.loccountrycode || "Unknown"
                        }</li>
                        <li>soloMMR: ${
                          data.mmr_estimate.estimate || "Unranked"
                        }</li>
                        <li>Rank: ${data.rank_tier || "Unranked"}</li>
                      </ul>
                    </div>`);
  $("header").empty().append(`<h1>DOTA 2 players finder</h1>
  <img src="./img/dotaLogo.png" alt="logo" />
  <form id="search-form">
    <input id="search-input" placeholder="Search by nickname" required />
    <button>Search</button>
  </form>`);
};

function showRecentMatches(data) {
  let recentMatch = "";
  for (let i = 0; i < 1; i++) {
    recentMatch += `<div class="center">
    <li>Last Match:</li>
